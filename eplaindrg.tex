%% This file contains some local (Whitman) changes to Eplain. The double
%% column macros have been fixed and enhanced to do multiple columns---3 and 4
%% columns are now possible. You can use \doublecolumns, \triplecolumns,
%% \quadcolumns, \begincolumns2, \begincolumns3, and \begincolumns4 to 
%% start multi-column modes, and \singlecolumn, \endcolumns or \begincolumns1 
%% to return to a single column.
%%
%% February 1996: added fudge factor \@djustvsize. A little too much stuff
%% seems to get collected sometimes in columns. By setting \@djustvsize
%% to a positive dimen, the vsize in column mode is reduced by that amount.
%%
%% Also added \l@stcolumnfill, a skip, that is inserted at the bottom
%% of a final column. This can be used to put a skip there if the last
%% column comes out too short, in which case the last column may be
%% stretched unappealingly. To use, for example, try:
%%
%%   \l@stcolumnfill 0pt plus1fill
%%
%% December 1996: Added \c@lumnfill, a skip, that is inserted at the bottom
%% of non-final columns. Setting \c@lumnfill 0pt plus1fill along with
%% the same for \l@stcolumnfill essentially
%% gives ragged bottoms to your columns. Also changed \vboxes to
%% \vtops  when packaging columns.
%%
%% October 1999: Added \flushcol to force the end of the current column,
%% roughly equivalent to \vfill\eject for pages.
%%
%% @texfile{
...
\def\xrefn#1{%
   ...
   \expandafter \ifx\the\toks0\relax
      ...
      \endgroup
   \fi
%   \special{html:<a href="\hyperh@sh\the\xrlabeltoks">}%
%   \the\toks0\special{html:</a>}    % Always produce something.
   \pdflink{\the\xrlabeltoks}%
   \the\toks0\endpdflink    % Always produce something.
}%
...
%
\newdimen\flushamt\newdimen\@necolsize
%\def\flushcol{\flushit=\vsize
%\divide\flushit by \number@fcolumns
%\ifdim\pagetotal>\flushit\flushit=2\flushit\fi
%\ifdim\pagetotal>\flushit\advance\flushit by \@necolsize\fi
%\ifdim\pagetotal>\flushit\flushit=\vsize\fi
%\advance\flushit by -\pagetotal
%\vfill\null\vskip\flushit}
...
\def\linkcolor{[0.0 0.0 0.7]}%
\def\urlcolor{[0.0 0.0 0.7]}%
\def\setlink#1{\colored{\urlcolor}{#1}}%
\def\pdflink#1{%
  \leavevmode\special{pdf: bann 
    << /Type /Annot /Subtype /Link /Border [ 0 0 0 ] /A << /S /GoTo /D (#1)
    >> >>}\begincolor{\linkcolor}}
%\def\url#1{%
%  \leavevmode\special{pdf: bann << /Type /Annot /Subtype /Link
%    /Border [ 0 0 0 ] /A << /Type /Action /S /URI /URI (#1) >>
%    >>}\begincolor{\urlcolor}}
\catcode`\@=11
\def\url{\leavevmode\begingroup\catcode`\~=12\catcode`\~=12\m@keurl}
\def\m@keurl#1{%
  \special{pdf: bann << /Type /Annot /Subtype /Link
  /Border [ 0 0 0 ] /A << /Type /Action /S /URI /URI (#1) >> >> }%
  \endgroup\begincolor{\urlcolor}}
\catcode`\@=12
\def\endurl{\endcolor\special{pdf: eann}\relax}
\let\endpdflink\endurl
