...
\defn The {\dfont natural logarithm} $\ln(x)$ is an antiderivative of
$1/x$, given by 
...
\htmlfigure{Transcendental-ln_definition.html}
\begincaption
$\ln(x)$ is an area.
\endcaption
\endfigure
...
\thm $\ds\lim_{x\to\infty} \ln x =\infty$
\endthm
...
\htmlfigure{Transcendental-ln_graph.html}
\begincaption
The graph of $\ln(x)$.
\endcaption
\endfigure
...
\example Let $f(x) =\ln (x^5 + 7x +12) $. Compute $f'(x) $.
...
\exercise Prove parts (d) and (e) of theorem~\xrefn{thm:log rules}.
\endexercise
...
\exercise Differentiate $f(x) =x\ln x $.
\endexercise
